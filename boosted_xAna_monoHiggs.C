...
void boosted_xAna_monoHiggs(std::string inputFile){
 ...
 // TCanvas* c2 = new TCanvas("c2","",889*1.5,768);
  TH1F* h_pfMet = new TH1F("h_pfMet", "phMet", 22,0,2200);
  TH1F* h_higgsPt = new TH1F("h_higgsPt", "higgs Pt", 15,0,1500);
  ...
  TH1F* h_higgsJetM = new TH1F("h_higgsJetMass", "higgs jet Mass", 20,100,200);
  int Hindex[2]={-1,-1};
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    //for(int i=0;i<2;i++){
      for(int j=0;j<2;j++){
            ...
            subjetP4[j]->SetPxPyPzE(subjetSDPx[i][j],subjetSDPy[i][j],subjetSDPz[i][j],subjetSDE[i][j]);
            //subjetP4[j]->SetPxPyPzE(subjetSDPx[j],subjetSDPy[j],subjetSDPz[j],subjetSDE[j]);
           }
